...
const Home: NextPage<People> = ({ people }) => {
  const blank = {name: '',  email: '', note: '', homePhone: '', cellPhone: '', country: '',  state: '',  city: '',  addressOne: '',  addressTwo: '',  schedule: '',  id: ''}
  ...
  async function updatePerson(name, note, email, homePhone, cellPhone, country, state, city, addressOne, addressTwo, id) {
    setForm({name, note, email, homePhone, cellPhone, country, state, city, addressOne, addressTwo, id})
    setNewPerson(false)
  }
            ...
            <li key={person.id} className="border-b border-gray-600 p-2">
              <div className="flex jusify-between">
                <div className="flex-1">
                  ...
                  <p className="text-sm">{person.homePhone}</p>
                  <p className="text-sm">{person.cellPhone}</p>
                  <p className="text-sm">{person.country}</p>
                  <p className="text-sm">{person.state}</p>
                  <p className="text-sm">{person.city}</p>
                  <p className="text-sm">{person.addressOne}</p>
                  <p className="text-sm">{person.addressTwo}</p>
                </div>
                <button onClick={() => updatePerson(person.name, person.note, person.email, person.homePhone, person.cellPhone, person.country, person.state, person.city, person.addressOne, person.addressTwo, person.id)}>Edit</button>
